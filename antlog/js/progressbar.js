	...
	$.getJSON(url + "?key=" + progress_key, function(data) {
		...
		if (parseInt(data.error) == 1) {
			...
			if (pathname.search(/\/\d+$/) == -1) {
				// path doesn't contain id, it must be part of the query string
				...
				// id is part of path
				var id = pathname.substr(pathname.search(/\/\d+$/));
				pathname = pathname.replace(/\/\d+$/, "/error" + id);
				$(location).attr('href', protocol + "//" + hostname + pathname + "?message=" + message);
			}
		}
